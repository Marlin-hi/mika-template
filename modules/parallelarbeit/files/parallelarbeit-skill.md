...
8. Branch + Worktree erstellen:
   ```bash
   # Worktree-Pfad relativ zum Vault — funktioniert auf allen Plattformen
   git worktree add ../[vault-name]-tabs/tab-XXXX -b tab-XXXX/aufgabenname
   ```

## Pfade

Worktrees landen immer unter `../[vault-name]-tabs/tab-XXXX` — ein Verzeichnis neben dem Vault. Keine hardcoded OS-Pfade verwenden. Das Script erkennt die Plattform automatisch:

```bash
VAULT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TABS_DIR="$(dirname "$VAULT_DIR")/[vault-name]-tabs"
```
